...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $del = $conn->prepare("DELETE FROM users WHERE id = ? AND role = 'non_teaching'");
    $del->bind_param("i", $id);
    if ($del->execute()) {
...
$stmt = $conn->prepare("SELECT * FROM users WHERE role = 'non_teaching' ORDER BY created_at DESC");
$stmt->execute();
...
<head>
    ...
    <title>Manage Non-Teaching Staff - Admin</title>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="6" class="text-center py-4">No non-teaching staff found.</td>
                                </tr>
                                <?php else: ?>
