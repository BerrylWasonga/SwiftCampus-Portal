...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_course'])) {
        ...
        $stmt->bind_param("ssis", $name, $code, $faculty_id, $level);
        if ($stmt->execute()) {
        ...
        } else $error = "Failed to add course: " . $conn->error;
        $stmt->close();
    }
    elseif (isset($_POST['update_course'])) {
        ...
        $stmt->bind_param("ssisi", $name, $code, $faculty_id, $level, $id);
        
        if ($stmt->execute()) $message = "Course updated successfully!";
        else $error = "Failed to update course: " . $conn->error;
        $stmt->close();
    }
    elseif (isset($_POST['delete_course'])) {
        ...
        else $error = "Failed to delete course: " . $conn->error;
        $stmt->close();
    }
}

// Fetch Courses with Hierarchy
$courses = $conn->query("SELECT c.*, f.faculty_name, f.id as faculty_id 
                         FROM courses c 
                         JOIN faculties f ON c.faculty_id = f.id 
                         ORDER BY f.faculty_name, c.course_name")->fetch_all(MYSQLI_ASSOC);
